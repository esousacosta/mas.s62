...
func tryNonce(nonce uint64, rows [][256]uint8, sigslice []Signature,
	byteMasks [32]byteMask) (bool, Signature, string) {
	msgString := fmt.Sprintf("Emanoel forge %d", nonce)
	msg := GetMessageFromString(msgString)
	forgedRows := pickRows(msg)

	// check only the bytes with fixed bits:
...
/*
	...
	<bit 0, row 0> <bit 1, row 0> <bit 2, row 0> ... <bit 255, row 0> <bit
	0, row 1> ...
	...
	need to use the same ordering as the ones I wrote in order to create
	the signatures
	here. I used what I thought was the most straightforward / simplest
	encoding, but
	...
	hash can be used instead, or both can be tried. This again interprets
	the message
	...
	for range numWorkers {
		go func() {
			...
			for nonce := range jobs {
				...
				ok, sig, msgString := tryNonce(nonce, rows, sigslice, byteMasks)
				if ok {
					select {
